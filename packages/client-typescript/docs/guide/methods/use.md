...
| `pipelineTraceLevel` | `str` / `string` | No | Trace level: `'none'`, `'metadata'`, `'summary'`, or `'full'`. Does not add a `_trace` field to this call's response -- when set, every lane write and invoke call is captured as an `apaevt_flow` event, retrievable as a full call tree through a `LogEventStream` monitor session (`client.log.open_event_stream(...)` / `client.log.openEventStream(...)`, via `get_traces()`/`get_trace(trace_id)`), and also rolled up into run analytics on the task's status object (`componentStats`, `slowestDocs`, `completionSeconds`, `idleSeconds`, `idleLongestSeconds`, `idleLongestAt`), retrieved via `get_task_status` / `getTaskStatus`. Neither is populated when no trace level is set. |
